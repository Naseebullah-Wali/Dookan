...
export const orderService = {
    ...
    async create(orderData) {
        // Prefer backend endpoint for order creation (server will verify reCAPTCHA and permissions)
        try {
            const resp = await api.post('/orders', orderData)
            return resp
        } catch (err) {
            // Fallback to direct Supabase insertion if backend API not available
            console.warn('Backend order creation failed, falling back to Supabase:', err.message)
            ...
            const total = orderInfo.subtotal + (orderInfo.shipping_fee || 0) + (orderInfo.tax || 0) - (orderInfo.discount || 0)

            const allowedMethods = ['cod', 'card', 'bank_transfer', 'paypal']
            const dbPaymentMethod = allowedMethods.includes(orderInfo.payment_method) ? orderInfo.payment_method : 'cod'
